import { decodeURIComponent as decode } from '../src/query/decodeURI';
import { run, bench } from 'mitata';
import { query as newParse } from '..';

const plusRegex = /\+/g;

function oldParse<T extends Record<string, any> = Record<string, string | string[]>>(input: string): T {
    let inputLength = input.length, result = {}, key = '', value = '', 
        startingIndex = -1, equalityIndex = -1, shouldDecodeKey = false, shouldDecodeValue = false, keyHasPlus = false, 
        valueHasPlus = false, hasBothKeyValuePair = false, charCode = 0, i = 0;
